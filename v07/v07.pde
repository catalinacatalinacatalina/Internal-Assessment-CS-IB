...
void mousePressed(){
  if(mousePressed){
    if(goToMiEquipo.mouseOverButton())
      pantalla = 1;
    if(goToPizarra.mouseOverButton())
      pantalla = 2;  
    if(goToResultados.mouseOverButton())
      pantalla = 3;
    if(goToGuardados.mouseOverButton())
      pantalla = 4;
    if(goToSobreNosotros.mouseOverButton())
      pantalla = 5;
    if(mouseOverLogo())
      pantalla = 0;
  }
  ...
  if(newPlayer.mouseOverButton()){
    pantalla=7;
    //nuevoJugador1.setDisp(true);
    //println("El pop up new player se encuentra en: "+ nuevoJugador1.getDisplay());
  }
  if(close.mouseOverButton()){
    //nuevoJugador1.setDisp(false);
  }
  if(prev.mouseOverButton() && prev.enability()){
    e.prev();
  } 
  if(next.mouseOverButton() && next.enability()){
    println("mouse over next");
    e.next();
  } 
  if(reset.mouseOverButton()){
    p.resetPinPositions();
  } 
  if(entrenamiento.mouseOverSelected()){
    println("ENTRENAMIENTO PRESSED");
    entrenamiento.setSelected(true);
    competicion.setSelected(!true);
  }
  if(competicion.mouseOverSelected()){
      competicion.setSelected(true);
      entrenamiento.setSelected(!true);
  } 
  if(mediaPista.mouseOverSelected()){
      println("mediaPista PRESSED");
      mediaPista.setSelected(true);
      pistaEntera.setSelected(!true);
  } 
  if(pistaEntera.mouseOverSelected()){
      pistaEntera.setSelected(true);
      mediaPista.setSelected(!true);
  }
  if(guardarJugador.mouseOverButton()){
